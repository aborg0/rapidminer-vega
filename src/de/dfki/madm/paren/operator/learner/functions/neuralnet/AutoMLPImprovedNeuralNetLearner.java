...
public class AutoMLPImprovedNeuralNetLearner extends AbstractLearner {
	...
	// /** The parameter name for &quot;The number of hidden layers. Only used if no layers are defined by the list
	// hidden_layer_types.&quot; */
	...
	// /** The parameter name for &quot;The learning rate determines by how much we change the weights at each
	// step.&quot; */
	// public static final String PARAMETER_LEARNING_RATE = "learning_rate";

	// /** The parameter name for &quot;The momentum simply adds a fraction of the previous weight update to the current
	// one (prevent local maxima and smoothes optimization directions).&quot; */
	...
	public List<ParameterType> getParameterTypes() {
		...
		types.add(type3);
		/*
		 * //hidden layers have been removed. - atif ParameterType type = new ParameterTypeList(PARAMETER_HIDDEN_LAYERS,
		 * "Describes the name and the size of all hidden layers.", new ParameterTypeString("hidden_layer_name",
		 * "The name of the hidden layer."), new ParameterTypeInt("hidden_layer_sizes",
	...
	private void quicksort(AutoMLPImprovedNeuralNetModel[] old_nn, double[] lR, int low, int high) {
		...
		while (i <= j) {
			...
			if (i <= j) {
				...
				Swap(old_nn, lR, i, j);
				/*
				 * AutoMLPImprovedNeuralNetModel temp = old_nn[i]; double l_temp = lR[i];
				 * 
				 * old_nn[i] = old_nn[j]; old_nn[j] = temp;
				 * 
				 * lR[i] = lR[j]; lR[j] = l_temp;
				 */
				i++;
				j--;
			}
