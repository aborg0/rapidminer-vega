...
public class RepositorySource extends AbstractReader<IOObject> {
	...
	public MetaData getGeneratedMetaData() throws OperatorException {
		...
		try {
			...
			addError(new SimpleProcessSetupError(Severity.WARNING, getPortOwner(), "repository_access_error", getParameterAsRepositoryLocation(PARAMETER_REPOSITORY_ENTRY), e.getMessage()));
		...
		if (entry != null) {
			try {
				...
				// We reduce the number of nominal values to a limit here to keep meta data transformations fast.
			...
			addError(new SimpleProcessSetupError(Severity.WARNING, getPortOwner(), "repository_location_does_not_exist", getParameterAsRepositoryLocation(PARAMETER_REPOSITORY_ENTRY)));
			return super.getGeneratedMetaData();			
		}
	}
