...
/** Reads an {@link ExampleSet} from an SQL {@link Connection} table. SQL datatypes
 *  are mapped to value types of {@link Attribute}s by using {@link DatabaseHandler#getRapidMinerTypeIndex(int)}.
 *  Data is copied into main memory.
 *  
 *  Data can be read from either a table name or a query. In the first case, the meta data is retrieved
 *  from the database meta data for that table for reasons of efficiency. In the latter case, a 
 *  LIMIT 0 is appended to the query, which may fail on some systems but which enables us to retrieve the
 *  structure of the table without performing the entire query itself. 
 *  
