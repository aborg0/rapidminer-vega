...
/**
 * <p>
 * This operator reads an example set from an URL. The format has to be a CSV format with
 * ';' as column separator and nominal values have to be quoted with a double quote (&quot;).
 * A quote inside of a nominal value has to be escaped by a backslash like in \&quot;. 
 * The first row is allowed to contain the column names which has to be indicated by 
 * the corresponding parameter. Comments are not allowed, unknown attribute values can be 
 * marked with empty strings or a question mark.
 * </p>
 * 
 * <p>
 * This operator is not nearly as powerful as the operators ExampleSource or SimpleExampleSource
 * but is on the other hand able to read data from arbitrary places as long as the format fits
 * the specification above. Please note also that the usage of this operator hardly allows for
 * a correct meta data description which might lead to problems if the meta data between training
 * and test set differ in a learning scenario.
 * </p>
 * 
 * <p>
 * Attribute roles can not be directly set during loading but the operator ChangeAttributeRole
 * has to be used after loading in order to change the roles.
 * </p>
 * 
...
public class URLExampleSource extends AbstractExampleSource {
	...
	public ExampleSet createExampleSet() throws OperatorException {
		...
		try {
			...
			while ((line = in.readLine()) != null) {
				...
				if ((rowLength >= 0) && (rowLength != row.length)) {
					if (skipErrorLines) {
						logWarning("Wrong number of columns in line " + lineCounter + ": was " + row.length + ", expected " + rowLength);
						continue;
					} else {
						throw new UserError(this, 302, getParameter(PARAMETER_URL), "Wrong number of columns in line " + lineCounter + ": was " + row.length + ", expected " + rowLength);
					}
				}
		...
		try {
			...
			guessValuesTypes.setParameter(GuessValueTypes.PARAMETER_DECIMAL_POINT_CHARACTER, Character.toString(decimalPointCharacter));
	...
	public List<ParameterType> getParameterTypes() {
		...
		ParameterType type = new ParameterTypeString(PARAMETER_COLUMN_SEPARATORS, "Column separators for data files (regular expression)", ",\\s*|;\\s*|\\s+");
