 ...
 * In contrast to the usual {@link MacroDefinitionOperator}, this operator supports the 
 * definition of a single macro from properties of a given input example set, e.g. from
 * properties like the number of examples or attributes or from a specific data value.</p>
 * 
...
public class DataMacroDefinitionOperator extends Operator {
	...
	public static final String[] STATISTICS_TYPES = new String[] {
		"average",
		"deviation",
		"variance",
		"min",
		...
		case MACRO_TYPE_STATISTICS:
			...
			case STATISTICS_TYPE_AVERAGE:
				if (attribute.isNominal()) {
					macroValue = attribute.getMapping().mapIndex((int)exampleSet.getStatistics(attribute, Statistics.MODE));
				} else {
			...
			case STATISTICS_TYPE_DEVIATION:
				if (!attribute.isNominal()) {
					...
					throw new UserError(this, 120, new Object[] { attribute.getName(), Ontology.VALUE_TYPE_NAMES[attribute.getValueType()], Ontology.VALUE_TYPE_NAMES[Ontology.NUMERICAL] });
				}
				break;
			...
			case STATISTICS_TYPE_MAX:
				if (attribute.isNominal()) {
					macroValue = attribute.getMapping().mapIndex((int)exampleSet.getStatistics(attribute, Statistics.MAXIMUM));
				} else {
			...
			case STATISTICS_TYPE_MIN:
				if (attribute.isNominal()) {
					macroValue = attribute.getMapping().mapIndex((int)exampleSet.getStatistics(attribute, Statistics.MINIMUM));
				} else {
	...
	public List<ParameterType> getParameterTypes() {
		...
		ParameterType type = new ParameterTypeCategory(PARAMETER_MACRO_TYPE, "Indicates the way how the macro should be defined.", MACRO_TYPES, MACRO_TYPE_EXAMPLES);
		type.setExpert(false);
		types.add(type);

		type = new ParameterTypeCategory(PARAMETER_STATISTICS, "The statistics of the specified attribute which should be used as macro value.", STATISTICS_TYPES, STATISTICS_TYPE_AVERAGE);
		type.setExpert(false);
		type.registerDependencyCondition(new EqualTypeCondition(this, PARAMETER_MACRO_TYPE, MACRO_TYPES, true, MACRO_TYPE_STATISTICS));
		types.add(type);

		type = new ParameterTypeAttribute(PARAMETER_ATTRIBUTE_NAME, "The name of the attribute from which the data should be derived.", exampleSetInput);
		type.setExpert(false);
		type.registerDependencyCondition(new EqualTypeCondition(this, PARAMETER_MACRO_TYPE, MACRO_TYPES, true, MACRO_TYPE_DATA, MACRO_TYPE_STATISTICS));
		types.add(type);
