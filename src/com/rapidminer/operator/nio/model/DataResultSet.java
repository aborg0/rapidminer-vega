 ...
 * It is just the interface for iterating over the raw data. The 
 * 
...
public interface DataResultSet {
	...
	/**
	 * Proceed to the next row if existent. Will throw NoSuchElementException if no further row exists
	 * @param listener 
	 * 
	 * @return
	 */
	...
	/**
	 * Returns the number of columns found so far. If there are any rows containing
	 * more columns that this value, an error has to be registered.
	 ...
	 * example the database column name might be returned.
	 * This method is only used, if there's no user specific setting in the {@link DataResultSetTranslationConfiguration} or annotations
