 ...
 * algorithms. The first subprocess contains the algorithm to be evaluated itself.
 * It must return an attribute weights vector which is then applied on the test data.
 * The same fold {@link XValidation} of the data is used to create a new model
 * during the second subprocess. This model is evaluated in the third subprocess which hence
 * has to return a performance vector. This performance vector serves as a performance
 * indicator for the actual algorithm. This implementation of a MethodValidationChain works similar to
...
public class WrapperXValidation extends WrapperValidationChain {
    ...
    public void doWork() throws OperatorException {
        ...
        SplittedExampleSet inputSet = new SplittedExampleSet(eSet, number, samplingType, getParameterAsBoolean(RandomGenerator.PARAMETER_USE_LOCAL_RANDOM_SEED), getParameterAsInt(RandomGenerator.PARAMETER_LOCAL_RANDOM_SEED), getCompatibilityLevel().isAtMost(SplittedExampleSet.VERSION_SAMPLING_CHANGED));
        ...
        for (iteration = 0; iteration < number; iteration++) {
            ...
            PerformanceVector iterationPerformance = evaluate(new AttributeWeightedExampleSet(newInputSet, weights, 0.0d).createCleanClone(), model);
    ...
    public List<ParameterType> getParameterTypes() {
        ...
        types.add(type);
        type = new ParameterTypeCategory(PARAMETER_SAMPLING_TYPE, "Defines the sampling type of the cross validation (linear = consecutive subsets, shuffled = random subsets, stratified = random subsets with class distribution kept constant)", SplittedExampleSet.SAMPLING_NAMES, SplittedExampleSet.STRATIFIED_SAMPLING, false);
        type.registerDependencyCondition(new BooleanParameterCondition(this, PARAMETER_LEAVE_ONE_OUT, true, false));
        types.add(type);
