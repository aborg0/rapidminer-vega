...
/**
 * <p>An operator chain that split an {@link ExampleSet} into two disjoint parts
...
public abstract class AbstractSplitChain extends OperatorChain {
	...
	private final PortPairExtender firstToSecondExtender = new PortPairExtender("through", getSubprocess(0).getInnerSinks(), getSubprocess(1).getInnerSources());

	private final OutputPort secondNestedExampleSetOutput = getSubprocess(1).getInnerSources().createPort("example set");

	private final PortPairExtender secondToOutputExtender = new PortPairExtender("through", getSubprocess(1).getInnerSinks(), getOutputPorts());
	...
	public AbstractSplitChain(OperatorDescription description) {
		...
		getTransformer().addRule(new ExampleSetPassThroughRule(exampleSetInput, firstNestedExampleSetOutput, SetRelation.EQUAL) {
			@Override
		...
		getTransformer().addRule(new ExampleSetPassThroughRule(exampleSetInput, secondNestedExampleSetOutput, SetRelation.EQUAL) {
			@Override
			public ExampleSetMetaData modifyExampleSet(ExampleSetMetaData metaData) throws UndefinedParameterError {
