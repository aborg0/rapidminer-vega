 ...
 * The IOObjects that are produced as output of the inner operators in the best cycle are then returned. The target of
 * this operator are methods that involve some non-deterministic elements such that the performance in each cycle may
 * vary. An example is k-means with random initialization.
 * 
...
public class RandomOptimizationChain extends SimpleOperatorChain {
	...
	private final InputPort innerPerformanceSink = getSubprocess(0).getInnerSinks().createPort("performance vector", PerformanceVector.class);
