...
/**
 * This operator is the base class for operators which loop over a file
 * structure.
 * 
 * @author Vaclav Uher
 */
public abstract class AbstractRepositoryIterator extends OperatorChain {
	...
	private final PortPairExtender inputExtender = new PortPairExtender("in",
			getInputPorts(), getSubprocess(0).getInnerSources());
	private final CollectingPortPairExtender outputExtender = new CollectingPortPairExtender(
    ...
    public static final String[] FILTERED_STRINGS = {"file name (last part of the path)", "full path (including file name)", "parent folder name"};
    ...
    abstract protected void iterate(Object currentParent, Pattern filter, boolean recursive, int type) throws OperatorException;
    
    protected void doWorkForSingleIterationStep(String fileName, String fullPath, String parentName, IOObject fileObject) throws OperatorException {
    	clearAllInnerSinks();
