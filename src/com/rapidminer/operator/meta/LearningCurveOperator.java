...
public class LearningCurveOperator extends OperatorChain {
	...
	private PortPairExtender throughExtender = new PortPairExtender("through", getSubprocess(0).getInnerSinks(), getSubprocess(1).getInnerSources());
	private InputPort performanceInnerSink = getSubprocess(1).getInnerSinks().createPort("performance", PerformanceVector.class);
	...
	public void doWork() throws OperatorException {
		...
		SplittedExampleSet trainTestSplittedExamples = new SplittedExampleSet(originalExampleSet, trainingRatio, samplingType, getParameterAsBoolean(RandomGenerator.PARAMETER_USE_LOCAL_RANDOM_SEED), getParameterAsInt(RandomGenerator.PARAMETER_LOCAL_RANDOM_SEED));
		...
		while (lastFraction <= 1.0d) {
			...
			trainTestSplittedExamples.selectSingleSubset(0);
			SplittedExampleSet growingTrainingSet = new SplittedExampleSet(trainTestSplittedExamples, lastFraction, samplingType, getParameterAsBoolean(RandomGenerator.PARAMETER_USE_LOCAL_RANDOM_SEED), getParameterAsInt(RandomGenerator.PARAMETER_LOCAL_RANDOM_SEED));
