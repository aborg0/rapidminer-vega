 ...
 * 
 * The operator is similar to {@link ParameterSetter}, but differs from that in not requiring a ParameterSet input. It
 ...
 * parameter which is optimized more than once within the optimization loop - ParameterSetter cannot be used here. <br/>
 * 
 * These parameters can either be generated by a {@link ParameterOptimizationOperator} or read by a
 * {@link com.rapidminer.operator.io.ParameterSetLoader}. This operator is useful, e.g. in the following scenario. If
 * one wants to find the best parameters for a certain learning scheme, one usually is also interested in the model
 * generated with this parameters. While the first is easily possible using a {@link ParameterOptimizationOperator}, the
 * latter is not possible because the {@link ParameterOptimizationOperator} does not return the IOObjects produced
 ...
 * model. <br/>
 * To solve this problem, one can use a <code>ParameterSetter</code>. Usually, a process definition with a
 * <code>ParameterSetter</code> contains at least two operators of the same type, typically a learner. One learner may
 * be an inner operator of the {@link ParameterOptimizationOperator} and may be named &quot;Learner&quot;, whereas a
 * second learner of the same type named &quot;OptimalLearner&quot; follows the parameter optimization and should use
 * the optimal parameter set found by the optimization. In order to make the <code>ParameterSetter</code> set the
 * optimal parameters of the right operator, one must specify its name. Therefore, the parameter list
 * <var>name_map</var> was introduced. Each parameter in this list maps the name of an operator that was used during
	...
	public int checkProperties() {
		...
		try {
			...
			if (nameList.size() > 0) {
				...
				errorNumber++;
				addError(new SimpleProcessSetupError(Severity.WARNING, this.getPortOwner(), Collections.singletonList(new ParameterSettingQuickFix(this, PARAMETER_NAME_MAP, "")), "parameter_list_undefined", "the mapping of operator names"));
			}
