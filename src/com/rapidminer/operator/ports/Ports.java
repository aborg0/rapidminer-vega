...
/** A collection of either input or output {@link Port}s (as defined by generic class T).
 *  <br/>
 *  Instances of this class serve as a factory and register of ports. Ports can be accessed
 *  by names or index. 
 *  Actual behaviour is defined by specialized interfaces {@link InputPorts} and {@link OutputPorts}.
 *  <br/>
 *  Implementors of operators are encouraged to generate their ports at construction time
 *  and keep private references to them, rather than retrieving them via one of the get methods every 
 *  time they are needed. The GUI will probably use the getter-methods.
 *  <br/>
 *  
...
public interface Ports<T extends Port> extends Observable<Port> {
	 ...
	 *  name should be a constant defined in the operator.
	 *  If a port with the given name does not exist, but a {@link PortExtender}
	 *  with a suitable prefix is registered, ports will be created accordingly.
	 *  In this case, the user must call {@link #unlockPortExtenders()} to 
	 *  guarantee that ports can be removed again. */
	...
	/** This is a backport method to generate IOContainers containing
	 *  all output objects of the given ports. */
