...
/**
 * This precondition must be added to the exampleset input port, if a distance measure might be chosen by the user. It
 * will check if the example set contains only compatible values.
 * 
			...
			try {
				...
				case DistanceMeasures.NOMINAL_MEASURES_TYPE:
					if (!containsOnlyType(emd, Ontology.NOMINAL, false)) {
						createError(Severity.ERROR, "measures.nominal", parameterHandler.getParameterAsString(DistanceMeasures.PARAMETER_NOMINAL_MEASURE));
					}
					break;
				case DistanceMeasures.NUMERICAL_MEASURES_TYPE:
					if (!containsOnlyType(emd, Ontology.NUMERICAL, false)) {
						createError(Severity.ERROR, "measures.numerical", parameterHandler.getParameterAsString(DistanceMeasures.PARAMETER_NUMERICAL_MEASURE));
					}
					break;
				case DistanceMeasures.DIVERGENCES_TYPE:
					if (!containsOnlyType(emd, Ontology.NUMERICAL, false)) {
						createError(Severity.ERROR, "measures.numerical", parameterHandler.getParameterAsString(DistanceMeasures.PARAMETER_DIVERGENCE));
					}
					break;
				}
