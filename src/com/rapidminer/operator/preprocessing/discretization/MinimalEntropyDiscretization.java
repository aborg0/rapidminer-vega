...
/**
 * <p>
 * This operator discretizes all numeric attributes in the dataset into nominal attributes. The discretization is
 * performed by selecting a bin boundary minimizing the entropy in the induced partitions. The method is then applied
 * recursively for both new partitions until the stopping criterion is reached. For Details see a) Multi-interval
 * discretization of continued-values attributes for classification learning (Fayyad,Irani) and b) Supervised and
 * Unsupervised Discretization (Dougherty,Kohavi,Sahami). Skips all special attributes including the label.
 * </p>
 * 
 * <p>
 * Please note that this operator automatically removes all attributes with only one range (i.e. those attributes which
 * are not actually discretized since the entropy criterion is not fulfilled). This behavior can be controlled by the
 * remove_useless parameter.
 * </p>
 * 
...
public class MinimalEntropyDiscretization extends AbstractDiscretizationOperator {
	...
	/**
	 * Indicates if useless discretized attributes, i.e. such attributes with only a single range after discretization
	 * should be removed.
	 */
	...
	protected void checkSelectedSubsetMetaData(ExampleSetMetaData subsetMetaData) {
		...
		case YES:
			...
			if (!labelMD.isNominal()) {
				getExampleSetInputPort().addError(new SimpleMetaDataError(Severity.ERROR, getExampleSetInputPort(), "attribute_has_wrong_type", labelMD.getName(), Ontology.VALUE_TYPE_NAMES[Ontology.NOMINAL]));
			}
			break;
		case NO:
			getExampleSetInputPort().addError(new SimpleMetaDataError(Severity.ERROR, getExampleSetInputPort(), "special_missing", "label"));
			break;
		case UNKNOWN:
		default:
			getExampleSetInputPort().addError(new SimpleMetaDataError(Severity.WARNING, getExampleSetInputPort(), "special_unknown", "label"));
			break;
		}
	}
	...
	private Double getMinEntropySplitpoint(LinkedList<double[]> truncatedExamples, Attribute label) {
		...
		int[] totalLabelDistribution = new int[label.getMapping().size()]; // Label distribution for all examples.
		...
		while (it1.hasNext()) { // Test every value as splitpoint
			...
			k1 = 0;
			k2 = 0;
			int[] labelDistribution1 = new int[label.getMapping().size()]; // Label distribution in class 1.
			int[] labelDistribution2 = new int[label.getMapping().size()]; // Label distribution in class 2.
			...
			while (it2.hasNext()) {
				...
				if (valueToCompare <= currentSplitpoint) {
					// Partition 1 gets all instances with values less or equal to the current splitpoint.
					s1++;
	...
	/*
	 * LinkedList partition consist of double arrays of size 2. array[0]=value of the current attribute,
	 * array[1]=corresponding label value.
	 */
	...
	/**
	 * Delivers the maximum range thresholds for all attributes, i.e. the value getRanges()[a][b] is the b-th threshold
	...
	public List<ParameterType> getParameterTypes() {
		...
		types.add(new ParameterTypeCategory(FrequencyDiscretization.PARAMETER_RANGE_NAME_TYPE, "Indicates if long range names including the limits should be used.", DiscretizationModel.RANGE_NAME_TYPES, DiscretizationModel.RANGE_NAME_LONG));

		ParameterType type = new ParameterTypeBoolean(FrequencyDiscretization.PARAMETER_AUTOMATIC_NUMBER_OF_DIGITS, "Indicates if the number of digits should be automatically determined for the range names.", true);
		type.registerDependencyCondition(new EqualTypeCondition(this, FrequencyDiscretization.PARAMETER_RANGE_NAME_TYPE, DiscretizationModel.RANGE_NAME_TYPES, false, DiscretizationModel.RANGE_NAME_INTERVAL));
		types.add(type);

		type = new ParameterTypeInt(FrequencyDiscretization.PARAMETER_NUMBER_OF_DIGITS, "The minimum number of digits used for the interval names (-1: determine minimal number automatically).", -1, Integer.MAX_VALUE, -1);
		type.registerDependencyCondition(new BooleanParameterCondition(this, FrequencyDiscretization.PARAMETER_AUTOMATIC_NUMBER_OF_DIGITS, false, false));
		types.add(type);
	...
	public ResourceConsumptionEstimator getResourceConsumptionEstimator() {
		return OperatorResourceConsumptionHandler.getResourceConsumptionEstimator(getExampleSetInputPort(), MinimalEntropyDiscretization.class, attributeSelector);
	}
}
