 ...
 * This discretization is performed by binning examples into bins of same size. The specified number 
 * of equally sized bins is created and the numerical values are simply sorted into
 * those bins, so that all bins contain the same number of examples. Skips all special attributes 
 * including the label.
 * 
...
public class AbsoluteDiscretization extends AbstractDiscretizationOperator {
	...
	protected Collection<AttributeMetaData> modifyAttributeMetaData(ExampleSetMetaData emd, AttributeMetaData amd) throws UndefinedParameterError {
		...
		if (getParameterAsInt(PARAMETER_RANGE_NAME_TYPE) == DiscretizationModel.RANGE_NAME_SHORT) {
			for (int i = 0; i < (int)Math.ceil(((double)emd.getNumberOfExamples().getNumber()) / getParameterAsInt(PARAMETER_SIZE_OF_BINS)); i++) {
	...
	public List<ParameterType> getParameterTypes() {
		...
		ParameterType type = new ParameterTypeInt(PARAMETER_SIZE_OF_BINS, "Defines the number of examples which should be used for each bin.", 1, Integer.MAX_VALUE, false);
		type.setExpert(false);
		types.add(type);

		types.add(new ParameterTypeCategory(PARAMETER_SORTING_DIRECTION, "Indicates if the values should be sorted in increasing or decreasing order.", SortedExampleSet.SORTING_DIRECTIONS, SortedExampleSet.DECREASING));
	...
	public ResourceConsumptionEstimator getResourceConsumptionEstimator() {
		String[] timeConsumption = OperatorResourceConsumptionHandler.getTimeConsumption(AbsoluteDiscretization.class);
		String[] memoryConsumption = OperatorResourceConsumptionHandler.getMemoryConsumption(AbsoluteDiscretization.class);
