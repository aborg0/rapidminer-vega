...
/**
 * This operator allows to adjust a date attribute by adding a constant value
 * in arbitrary units as days, hours or seconds to the attributes value.
 * 
 * @author Ingo Mierswa
 */
public class DateAdjust extends AbstractDataProcessing {
	...
	public static final String[] CALENDAR_FIELDS = new String[] {
		"Year",
		"Month",
		"Day",
		"Hour",
	...
	public DateAdjust(OperatorDescription description) {
		...
		getExampleSetInputPort().addPrecondition(new AttributeSetPrecondition(getExampleSetInputPort(), AttributeSetPrecondition.getAttributesByParameter(this, PARAMETER_ATTRIBUTE_NAME), Ontology.DATE_TIME));
	}
	...
	public ExampleSet apply(ExampleSet exampleSet) throws OperatorException {		
		...
		if (!Ontology.ATTRIBUTE_VALUE_TYPE.isA(dateAttribute.getValueType(), Ontology.DATE_TIME)) {
			throw new UserError(this, 120, attributeName, new Object[] { Ontology.VALUE_TYPE_NAMES[dateAttribute.getValueType()], Ontology.VALUE_TYPE_NAMES[Ontology.DATE_TIME] });
		}
		...
		// store adjustments
		List<Adjustment> adjustments = new LinkedList<Adjustment>();
	...
	public List<ParameterType> getParameterTypes() {
		...
		types.add(new ParameterTypeAttribute(PARAMETER_ATTRIBUTE_NAME, "The attribute which should be parsed.", getExampleSetInputPort(), false, Ontology.DATE_TIME));

		types.add(new ParameterTypeList(PARAMETER_ADJUSTMENTS, "This list defines all date adjustments.",
				new ParameterTypeInt("adjustment", "The number of units to add to the dates.", -Integer.MAX_VALUE, Integer.MAX_VALUE),
				new ParameterTypeCategory(PARAMETER_DATE_UNIT, "The unit which should be adjusted.", CALENDAR_FIELDS, CALENDAR_FIELD_HOUR)));
