...
/**
 * This condition filters out all attributes whose block type does not match the one
 * given in the parameter block_type. This can be
 * useful e.g. for preprocessing operators that can handle only series
 * attributes. Features of the type mentioned in except_block_type are
 * removed even if they match block_type.
 * 
...
public class BlockTypeAttributeFilter extends AbstractAttributeFilterCondition {
	...
	public List<ParameterType> getParameterTypes(ParameterHandler operator, InputPort inPort, int...valueTypes) {
		...
		ParameterType type = new ParameterTypeCategory(PARAMETER_EXCEPT_BLOCK_TYPE, "Except this block type.", exceptValueTypeNames, exceptValueTypeNames.length - 1, true);
