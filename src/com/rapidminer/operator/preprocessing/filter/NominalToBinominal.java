...
public class NominalToBinominal extends PreprocessingOperator {
			...
			if (!transformBinominal && (amd.getValueSet().size() == 2)) {
				...
				if (amd.getValueSetRelation() != SetRelation.UNKNOWN) {
					for (String value : amd.getValueSet()) {
						...
						(getParameterAsBoolean(PARAMETER_USE_UNDERSCORE_IN_NAME) ? "_" : " = ") +
						value;
	...
	public ResourceConsumptionEstimator getResourceConsumptionEstimator() {
		return OperatorResourceConsumptionHandler.getResourceConsumptionEstimator(getInputPort(), NominalToBinominal.class, attributeSelector);
	}
