...
/**
 * This operator simply replaces all values by their absolute
 * respective value.
 * 
	...
	public ExampleSetMetaData applyOnFilteredMetaData(ExampleSetMetaData metaData) {
		for (AttributeMetaData amd: metaData.getAllAttributes()) {
			if (amd.isNumerical() && !amd.isSpecial()) {
				...
				amd.setValueRange(new Range(0, Math.max(Math.abs(range.getLower()), Math.abs(range.getUpper()))), amd.getValueSetRelation());
