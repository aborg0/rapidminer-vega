...
public class KennardStoneSampling extends AbstractSamplingOperator {
	...
	public ExampleSet apply(ExampleSet exampleSet) throws OperatorException {	 
		...
		for (Example example: exampleSet) {
			...
			Candidate current = new Candidate(exampleValues, Math.abs(distanceMeasure.calculateDistance(meanVector, exampleValues)), i);
		...
		while (numberOfSelectedExamples < size) {
			...
			for (Example example: exampleSet) {
				...
				if (partition[i] == 0) {
					...
					for (Candidate candidate: recentlySelected) {
						minimalDistances[i] = Math.min(minimalDistances[i], Math.abs(distanceMeasure.calculateDistance(exampleValues, candidate.getValues())));
					}
			...
			while (lit.hasPrevious()) {
				...
				while (addedIterator.hasNext()) {
					double distance = Math.abs(distanceMeasure.calculateDistance(addedIterator.next().getValues(), candidate.getValues()));
	...
	public ResourceConsumptionEstimator getResourceConsumptionEstimator() {
		return OperatorResourceConsumptionHandler.getResourceConsumptionEstimator(getInputPort(), KennardStoneSampling.class, null);
	}
}
