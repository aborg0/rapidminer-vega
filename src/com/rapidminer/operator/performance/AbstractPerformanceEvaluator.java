...
public abstract class AbstractPerformanceEvaluator extends Operator implements CapabilityProvider {
     ...
     * 
     *  ATTENTION! This method is called during the creation of parameters. Do not try to
     *  get a parameter value inside this method, since this will create an infinite loop!
     */
    ...
    /** Helper method if this operator is constructed anonymously. Assigns the
     *  exampleSet to the input and returns the PerformanceVector from the output.
     */
    ...
    /**
     * This will override the checkProperties method of operator in order to add some checks for possible
     * quickfixes.
     */
    @Override
    public int checkProperties() {
        ...
        if (!criterionChecked && criteria != null) {
            ...
            if (criteria.size() > 0) {
                quickFixes.add(new ParameterSettingQuickFix(AbstractPerformanceEvaluator.this, getCriteria().get(0).getName(), "true"));
                addError(new SimpleProcessSetupError(Severity.ERROR, this.getPortOwner(), quickFixes, "performance_criterion_undefined", criteria.get(0).getName()));
            }
            if (criteria.size() > 1)
