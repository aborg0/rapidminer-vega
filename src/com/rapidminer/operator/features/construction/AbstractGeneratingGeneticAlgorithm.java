...
public abstract class AbstractGeneratingGeneticAlgorithm extends ExampleSetBasedFeatureOperator {
	 ...
	 */
	protected abstract ExampleSetBasedPopulationOperator getGeneratingPopulationOperator(ExampleSet exampleSet) throws OperatorException;

	/**
	 * Returns an operator that performs the mutation. Can be overridden by
	 * subclasses.
	 */
	protected abstract ExampleSetBasedPopulationOperator getMutationPopulationOperator(ExampleSet example) throws OperatorException;
	...
	public final List<ExampleSetBasedPopulationOperator> getPostEvaluationPopulationOperators(ExampleSet input) throws OperatorException {
		...
		this.generationsWithoutImproval = getParameterAsBoolean(PARAMETER_USE_EARLY_STOPPING) ? getParameterAsInt(PARAMETER_GENERATIONS_WITHOUT_IMPROVAL) : maxGen;
		...
		postOp.add(new ExampleSetBasedTournamentSelection(numberOfIndividuals, getParameterAsDouble(PARAMETER_TOURNAMENT_SIZE), this.maxGen, dynamicSelection, keepBest, getRandom()));
	...
	public List<ParameterType> getParameterTypes() {
		...
		type = new ParameterTypeDouble(PARAMETER_P_CROSSOVER, "Probability for an individual to be selected for crossover.", 0, 1, 0.5);
		types.add(type);
