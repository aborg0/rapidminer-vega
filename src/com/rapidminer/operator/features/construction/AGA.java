 ...
 * {@link com.rapidminer.operator.features.construction.GeneratingGeneticAlgorithm} operator. 
...
public class AGA extends GeneratingGeneticAlgorithm {
	...
	@Override
	protected List<ExampleSetBasedPopulationOperator> getPreProcessingPopulationOperators(ExampleSet input) throws OperatorException {
		...
		if (maxPeaks > 0)
			popOps.add(new FourierGenerator(maxPeaks, getParameterAsInt(PARAMETER_ADAPTION_TYPE), getParameterAsInt(PARAMETER_ATTRIBUTES_PER_PEAK), getParameterAsDouble(PARAMETER_EPSILON), getRandom()));
		return popOps;
	}
