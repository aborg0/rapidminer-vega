...
/**
 * This operator performs a Singular Value Decomposition (SVD) of the data
 * The user can specify the number of target dimensions
 * operator outputs a {@link SVDModel}. With the <code>ModelApplier</code> you can transform the features.
 * 
...
public class SVDReduction extends Operator {
    ...
    public List<ParameterType> getParameterTypes() {
        ...
        ParameterType type = new ParameterTypeCategory(PARAMETER_REDUCTION_TYPE, "Indicates which type of dimensionality reduction should be applied", REDUCTION_METHODS, REDUCTION_FIXED);
        type.setExpert(false);
        list.add(type);

        type = new ParameterTypeDouble(PARAMETER_PERCENTAGE_THRESHOLD, "Keep the all components with a cumulative variance smaller than the given threshold.", 0, 1, 0.95);
        type.setExpert(false);
        type.registerDependencyCondition(new EqualTypeCondition(this, PARAMETER_REDUCTION_TYPE, REDUCTION_METHODS, true, REDUCTION_PERCENTAGE));
        list.add(type);
