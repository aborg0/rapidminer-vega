...
/**
 * This operator performs the independent componente analysis (ICA). Implementation of the FastICA-algorithm of
 * Hyvaerinen und Oja. The operator outputs a <code>FastICAModel</code>. With the <code>ModelApplier</code> you can
...
public class FastICA extends Operator {
	...
	/**
	 * The parameter name for &quot;If 'parallel' the components are extracted simultaneously, 'deflation' the
	...
	/**
	 * The parameter name for &quot;constant in range [1, 2] used in approximation to neg-entropy when
	 * fun="logcosh"&quot;
	 */
	...
	public FastICA(OperatorDescription description) {
		...
		exampleSetInput.addPrecondition(new ExampleSetPrecondition(exampleSetInput, Ontology.NUMERICAL) {
			...
			public void makeAdditionalChecks(ExampleSetMetaData emd) throws UndefinedParameterError {
				...
				if (desiredComponents > emd.getNumberOfRegularAttributes() && getParameterAsInt(PARAMETER_REDUCTION_TYPE) == REDUCTION_FIXED) {
	...
	public List<ParameterType> getParameterTypes() {
		...
		ParameterType type = new ParameterTypeCategory(PARAMETER_REDUCTION_TYPE, "Indicates which type of dimensionality reduction should be applied", REDUCTION_METHODS, REDUCTION_NONE);
