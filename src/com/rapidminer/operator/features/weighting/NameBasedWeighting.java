 ...
 * with "Att" to the weight 0.5 by using the regular expression "Att.*". Alternatively,
 * the specified weight may be considered as weight sum for all attributes matching the
 * corresponding regular expression and may be equally distributed among these attributes.
 * All other feature  weights whose feature names are not covered by one of the regular
 * expressions are set to the default weight.</p>
 * 
...
public class NameBasedWeighting extends AbstractWeighting {
	...
	public List<ParameterType> getParameterTypes() {
		...
		ParameterType type = new ParameterTypeList(PARAMETER_ATTRIBUTE_NAME_REGEX, "This list maps different regular expressions for the feature names to the specified weights.", 
											       new ParameterTypeRegexp("attributes", "This regular expression specifies the attributes to assign the weight.") {
													   private static final long serialVersionUID = -6782736474400842376L;

													   @Override
													   public LinkedList<String> getPreviewList() {
														   LinkedList<String> previewList = new LinkedList<String>();
												   ...
												   new ParameterTypeDouble("weight", "The new weight for all this attributes.", Double.NEGATIVE_INFINITY, Double.MAX_VALUE, false));
