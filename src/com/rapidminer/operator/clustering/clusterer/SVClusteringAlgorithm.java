...
public class SVClusteringAlgorithm implements SVMInterface {
	...
	private static final int[] RAPID_MINER_VERBOSITY = {
			LogService.STATUS, LogService.STATUS, LogService.STATUS, LogService.MINIMUM, LogService.MINIMUM
	};
	...
	protected void optimize() {
		...
		while (KKTerror) {
			...
			if (new_target < old_target) {
				...
				// nothing we can do
				logln(5, "WARNING: no descend (" + (old_target - new_target) + " <= " + descend + "), stopping (it = " + target_count + ").");
				KKTerror = false;
				convError = true;
			};
		};
