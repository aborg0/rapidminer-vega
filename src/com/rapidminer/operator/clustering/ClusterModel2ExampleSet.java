...
/**
 * This Operator clusters an exampleset given a cluster model. If an exampleSet does not 
 * contain an id attribute it is probably not the same as the cluster model has been created on.
 * Since cluster models depend on a static nature of the id attributes, the outcome on another exampleset
 * with different values but same ids will be unpredictable and hence not automatically creation of ids is performed. 
 * Only centroid based clusterings support assiging unseen examples to clusters. * 
...
public class ClusterModel2ExampleSet extends Operator {
	...
	public ClusterModel2ExampleSet(OperatorDescription description) {
		super(description);
		getTransformer().addRule(new AttributeAddingExampleSetPassThroughRule(exampleSetInput, exampleSetOutput, new AttributeMetaData(Attributes.CLUSTER_NAME, Ontology.NOMINAL, Attributes.CLUSTER_NAME)));
	...
	public List<ParameterType> getParameterTypes() {
		...
		ParameterType type = new ParameterTypeBoolean(PARAMETER_ADD_AS_LABEL, "Should the cluster values be added as label.", false);
