...
/**
 * <p>This is a regularized discriminant analysis (RDA) which is a generalization of the LDA or QDA.
 * Both algorithms are special cases of this algorithm, using parameter alpha = 1 respectively alpha = 0.</p>
 * 
...
public class RegularizedDiscriminantAnalysis extends LinearDiscriminantAnalysis {
	...
	protected Matrix[] getInverseCovarianceMatrices(ExampleSet exampleSet, String[] labels) throws UndefinedParameterError {
		...
		for (String label: labels) {
			...
			Matrix inverse = MathFunctions.invertMatrix(CovarianceMatrix.getCovarianceMatrix(labelSet));
		...
		for (int i = 0; i < labels.length; i++) {
			regularizedMatrices[i] = globalInverseCovariances[i].times(alpha).plus(classInverseCovariances[i].times(1d - alpha));
		}
	...
	protected DiscriminantModel getModel(ExampleSet exampleSet, String[] labels, Matrix[] meanVectors, Matrix[] inverseCovariances, double[] aprioriProbabilities) throws UndefinedParameterError {
		return new DiscriminantModel(exampleSet, labels, meanVectors, inverseCovariances, aprioriProbabilities, getParameterAsDouble(PARAMETER_ALPHA));		
	}
	...
	public List<ParameterType> getParameterTypes() {
		...
		list.add(new ParameterTypeDouble(PARAMETER_ALPHA, "This is the strength of regularization. 1: Only global covariance is used, 0: Only per class covariance is used.", 0d, 1d, 0.5d, false));
