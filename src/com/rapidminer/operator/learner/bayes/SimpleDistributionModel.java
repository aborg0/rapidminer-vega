...
/**
 * DistributionModel is a model for learners which estimate distributions of attribute values from example sets like
 * NaiveBayes.
 * 
 * Predictions are calculated as product of the conditional probabilities for all attributes times the class
 * probability.
 * 
 * The basic learning concept is to simply count occurrences of classes and attribute values. This means no
 * probabilities are calculated during the learning step. This is only done before output. Optionally, this calculation
 * can apply a Laplace correction which means in particular that zero probabilities are avoided which would hide
 * information in distributions of other attributes.
 * 
...
public class SimpleDistributionModel extends DistributionModel {
	...
	/**
	 * Specifies the total weight of examples in which the different combinations of classes and (nominal) attribute
	 * values co-occur. In the case of numeric attributes the (weighted) sum and the (weighted) sum of the squared
	 * attribute values are stored which are needed to calculate the mean and the standard deviation/variance of the
	 * resulting (assumed) normal distribution.
	 * 
	 * Array dimensions: 1st: attributes 2nd: classes 3nd: nominal values or value sum (index=0) and squared value sum
	 * (index=1)
	 */
	 ...
	 * given the class value for nominal values. Contains the means and standard deviations for numerical attributes.
	 * 
	 * Array dimensions: 1st: attributes 2nd: classes 3nd: nominal values or mean (index=0) and standard deviation
	 * (index=1)
	 */
	...
	/**
	 * This constructor allows to build a distribution model from the given data characteristics. It is fully updateable.
	 * For details on weightsSums, please take a look at the member variable weightSums. The ExampleSet is only used for
	 * storing the header. The attributes and their values, including the class values, must be in the same order in the
	 * headerSet as they are in the encoded in the weight sums.
	 */
	...
	/**
	 * This constructor will derive a complete distribution model on basis of the given trainings data with Laplace
	 * correcture enabled.
	 */
	...
	/**
	 * This constructor will derive a complete distribution model on basis of the given trainings data with Laplace
	 * correcture depending on the parameter.
	 */
	...
	/**
	 * Updates the model by counting the occurrences of classes and attribute values in combination with the class
	 * values.
	 * 
				...
				for (Attribute attribute : exampleSet.getAttributes()) {
					...
					if (nominal[attributeIndex]) {
						...
						if (!Double.isNaN(attributeValue)) {
							...
							// these are used to distinguish between total class weights and the current attribute's weights
	...
	/**
	 * Updates the distribution properties by calculating the logged probabilities and distribution parameters on the
	 * basis of the weight counters.
	 */
		...
		for (int i = 0; i < numberOfAttributes; i++) {
			if (nominal[i]) {
				...
				for (int j = 0; j < numberOfClasses; j++) {
					...
					double standardDeviationSquared = (weightSums[i][j][INDEX_SQUARED_VALUE_SUM] - weightSums[i][j][INDEX_VALUE_SUM] * weightSums[i][j][INDEX_VALUE_SUM] / classWeight) / (classWeight - 1);
