...
public class Hypothesis implements Serializable {
	...
	public Hypothesis() {
		literalMap = new LinkedHashMap<Attribute,Literal>();
	}
	...
	private Rule getPredictionRule(Attribute label) {
		double predictionIndex = positiveWeight / coveredWeight > 0.5 ? label.getMapping().getPositiveIndex() : label.getMapping().getNegativeIndex();
