...
/**
 * PSO approach for SVM optimization. Currently only classification problems are
 * supported.
 * 
 * @author Ingo Mierswa
 *          Exp $
...
public class PSOSVMOptimization extends PSOOptimization {
	...
	public PSOSVMOptimization(ExampleSet exampleSet, // training data
			...
			int maxIterations, int generationsWithoutImprovement, // convergence
																	// paras
			int popSize, double inertiaWeight, double localWeight, double globalWeight, boolean dynamicInertiaWeight, boolean showPlot, RandomGenerator random) {
		super(popSize < 1 ? exampleSet.size() : popSize, exampleSet.size(), maxIterations, generationsWithoutImprovement, inertiaWeight, localWeight, globalWeight, 0.0d, 1.0d, dynamicInertiaWeight, random);
	...
	public void nextIteration() {
		if (showPlot)
			dataTable.add(new SimpleDataTableRow(new double[] { getGeneration(), getBestFitnessEver(), getBestFitnessInGeneration() }));
	}
