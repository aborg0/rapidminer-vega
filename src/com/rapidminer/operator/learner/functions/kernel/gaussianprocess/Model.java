...
public class Model implements Serializable {
	...
	/*
	 * The vector k_t+1 = [K(x_1, x_t+1), K(x_2, x_t+1), ..., K(x_t, x_t+1)], holding the results of using the kernel on
	 * the the new input x_t+1 and the rest of the old inputs
	 */
	...
	/**
	 * Apply the model to a (new) input vector x_t+1 in order to get a prediction, which - as a GP-marignal at x_t+1 -
	 * is a one-dimensional gaussian distribution with mean m and covariance sigma^2 (2.22, the parameterisation lemma).
     * Returns only the function value, the mapping to a classification must be done by the invoking method.
	 */
	...
	public double applyToVector(double[] x_new) {
        ...
        /* IM: should be done by invoking method
		if (!regression) {
			if (prediction > 0.0)
				prediction = 1.0;
			else
