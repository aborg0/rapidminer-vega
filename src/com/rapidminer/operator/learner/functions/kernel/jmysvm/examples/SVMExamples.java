...
public class SVMExamples implements Serializable {
	...
	/**
	 * This class holds information aboud the means and variances of an
	 * attribute. This is needed to use the same values for the test set if
	 * scaling is performed by an SVM operator instead of using a preprocessing
	 * step.
	 */
	 ...
	 * known.
	 */
	public SVMExamples(com.rapidminer.example.ExampleSet exampleSet, Attribute labelAttribute, Map<Integer, MeanVariance> meanVariances) {
	...
	public String toString(int numberOfExamples, boolean onlySV) {
		StringBuffer result = new StringBuffer("SVM Example Set (" + (onlySV ? (getNumberOfSupportVectors() + " support vectors") : (train_size + " examples")) + "):"+Tools.getLineSeparator()+"b: " + b + Tools.getLineSeparator());
