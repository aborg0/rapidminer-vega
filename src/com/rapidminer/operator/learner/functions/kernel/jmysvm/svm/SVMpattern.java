...
public class SVMpattern extends SVM {
	...
	public SVMpattern(Operator paramOperator, Kernel kernel, SVMExamples sVMExamples, com.rapidminer.example.ExampleSet rapidMinerExamples, RandomGenerator randomGenerator) throws UndefinedParameterError {
	...
	protected void optimize() {
		...
		while (KKTerror) {
			...
			if (new_target < old_target) {
				...
				logln(5, "WARNING: no descend (" + (old_target - new_target) + " <= " + descend + "), adjusting is_zero to " + my_is_zero);
	...
	protected void print_statistics() {
		...
		if (do_xi_alpha) {
			...
			logln(1, "Xi-Alpha Precision" + (((double) (total_pos - estim_pos)) / ((double) (total_pos - estim_pos + estim_neg))));
