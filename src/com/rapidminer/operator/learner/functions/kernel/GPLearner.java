...
//			return true;
	...
	public Model learn(ExampleSet exampleSet) throws OperatorException {
		...
		/* Tolerance value: we project the current basis vector if it has 
		 * a orthogonal distance to the linear span of the other 
		 * basis vectors smaller than epsilon_tol */
	...
	public List<ParameterType> getParameterTypes() {
		...
		type = new ParameterTypeDouble(PARAMETER_KERNEL_SHIFT, "The SVM kernel parameter shift (Multiquadric).", Double.NEGATIVE_INFINITY, Double.POSITIVE_INFINITY, 1.0d);
		...
		type = new ParameterTypeDouble(PARAMETER_EPSILON_TOL, "Tolerance for gamma induced projections", 0, Double.POSITIVE_INFINITY, 1e-7);
		type.setExpert(true);
		types.add(type);

		type = new ParameterTypeDouble(PARAMETER_GEOMETRICAL_TOL, "Tolerance for geometry induced projections", 0, Double.POSITIVE_INFINITY, 1e-7);
		type.setExpert(true);
		types.add(type);
