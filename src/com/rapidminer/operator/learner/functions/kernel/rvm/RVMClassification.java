...
public class RVMClassification extends RVMBase {
	...
	public Model learn() {
    	...
    	int prune_point		= 50;														// Iteration number after which we switch to analytic pruning (in percent)
    	...
    	for (i = 1; i <= parameter.maxIterations; i++) {
    		...
    		/** Use the iterative reweighted least algorithm (IRLS) to find the maximum w_mp of the marginal likelihood 
    		...
    		for (j = 0; j < 25; j++) {
    			...
    			/** Calculate IRLS-Hessian:
    			 *  Hessian = X' * W * X 
    			 *          = PHI' * W_IRLS * PHI 
    			 *          = (W_IRLS * PHI)' * PHI 
    			 */
    			
        		Matrix matrixIRLSWeights	= new Matrix(prunedMatrixPHI.getRowDimension(), prunedMatrixPHI.getRowDimension(), 0);
...
//        			if (cond < Math.pow(2, -52)) {					// Compare condition to mantissa-precision of the "double"-type
//        				LogService.logMessage("(IRLS) ill-conditioned hession: cond = " + (new Double(cond)).toString(), LogService.STATUS);
//        				LogService.logMessage("(IRLS) returning back to hyperparameter estimation ...", LogService.STATUS);
//        				
