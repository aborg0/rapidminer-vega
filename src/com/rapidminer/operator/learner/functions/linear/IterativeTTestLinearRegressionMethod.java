...
/**
 * This implements an iterative T-Test based selection. First a forward
 * selection is run
 * and all attributes for which the null hypothesis is significantly denied are
 * selected. On
 * this set a backward selection is performed and all attributes for which in
 * the combination the null hypothesis
 * can't be denied are dropped. The next round then starts with the remaining
 * attributes until
 * there's no further change or the maximal number of rounds are exceeded.
 * 
...
public class IterativeTTestLinearRegressionMethod extends TTestLinearRegressionMethod {
        ...
        while (iteration == 0 || (iteration < maxIterations && isSelectionDiffering(isUsedAttribute, isLastRoundUsed))) {
            ...
            for (int i = 0; i < isAllowedToUse.length; i++) {
                if (isAllowedToUse[i] && !isUsedAttribute[i]) {
                    ...
                    double pValue = getPValue(coefficients[coefficientIndex], i, regression, useBias, ridge, exampleSet, isUsedAttribute, standardDeviations, labelStandardDeviation, fdistribution, generalCorrelation);
            ...
            {
                double[] coefficients = regression.performRegression(exampleSet, isUsedAttribute, means, labelMean, ridge);
                isUsedAttribute = filterByPValue(regression, useBias, ridge, exampleSet, isUsedAttribute, means, labelMean, standardDeviations, labelStandardDeviation, coefficients, alphaBackward).isUsedAttribute;
            }
    ...
    public List<ParameterType> getParameterTypes() {
        ...
        types.add(new ParameterTypeInt(PARAMETER_MAX_ITERATIONS, "The maximal number of rounds for iterative selection.", 1, Integer.MAX_VALUE, 10));
        types.add(new ParameterTypeDouble(PARAMETER_FORWARD_SELECTION_THRESHOLD, "This is the alpha level for the used t-test for selecting attributes.", 0, 1, 0.05));
        types.add(new ParameterTypeDouble(PARAMETER_BACKWARD_SELECTION_THRESHOLD, "This is the alpha level for the used t-test for deselecting attributes.", 0, 1, 0.05));
        return types;
    }
