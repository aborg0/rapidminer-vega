		...
		while (improved) {
			...
			for (int i = 0; i < isUsedAttribute.length; i++) {
				if (isUsedAttribute[i]) {
					double standardizedCoefficient = Math.abs(coefficientsOnFullData[coefficientIndex] * standardDeviations[i] / labelStandardDeviation);
			...
			if (attribute2Deselect >= 0) {
				...
				double[] currentCoefficients = regression.performRegression(exampleSet, isUsedAttribute, means, labelMean, ridge);
