...
public class LinearRegression extends AbstractLearner {
    ...
    public Model learn(ExampleSet exampleSet) throws OperatorException {
        ...
        Matrix productMatrix = matrixT.times(matrix);
        Matrix invertedMatrix = null;
        try {
            // try to invert matrix
            invertedMatrix = productMatrix.inverse();
            ...
            for (int i = 0; i < result.isUsedAttribute.length; i++) {
                if (result.isUsedAttribute[i]) {
                    ...
                    standardErrors[index] = Math.sqrt(mse * invertedMatrix.get(index + 1, index + 1));
                    standardizedCoefficients[index] = result.coefficients[index] * standardDeviations[i] / means[i];
            ...
            for (int i = 0; i < result.isUsedAttribute.length; i++) {
                if (result.isUsedAttribute[i]) {
                    ...
                    standardErrors[index] = Math.sqrt((1.0d - generalCorrelation) / (tolerance * (exampleSet.size() - exampleSet.getAttributes().size() - 1.0d))) * labelStandardDeviation / standardDeviations[i];
        ...
        if (invertedMatrix == null) {
            ...
            standardErrors[standardErrors.length - 1] = Math.sqrt(mse * invertedMatrix.get(0, 0));
        }
        ...
        if (!Tools.isZero(standardErrors[standardErrors.length - 1])) {
            tStatistics[tStatistics.length - 1] = result.coefficients[result.coefficients.length - 1] / standardErrors[standardErrors.length - 1];
            double probability = fdistribution.getProbabilityForValue(tStatistics[tStatistics.length - 1] * tStatistics[tStatistics.length - 1]);
        ...
        return new LinearRegressionModel(exampleSet, result.isUsedAttribute, result.coefficients, standardErrors, standardizedCoefficients, tolerances, tStatistics, pValues, useBias, firstClassName, secondClassName);
    }

    double getTolerance(ExampleSet exampleSet, boolean[] isUsedAttribute, int testAttributeIndex, double ridge, boolean useIntercept) throws UndefinedParameterError {
    ...
    /**
     * Calculate a linear regression only from the selected attributes. The
     * method returns the calculated coefficients.
     */
     ...
     * method returns the calculated coefficients.
     */
    double[] performRegression(ExampleSet exampleSet, Attribute[] usedAttributes, Attribute label, double ridge) throws UndefinedParameterError {
        Matrix independent = null;
        Matrix dependent = null;
        double[] weights = null;
        ...
        for (int i = 0; i < usedAttributes.length; i++) {
            coefficients[coefficients.length - 1] -= coefficients[i] * exampleSet.getStatistics(usedAttributes[i], Statistics.AVERAGE);
        }
