 ...
 * learning steps to build its model. New meta learning schemes should extend
...
public abstract class AbstractMetaLearner extends OperatorChain implements Learner {
	...
	public AbstractMetaLearner(OperatorDescription description) {
		...
		innerModelSink.addPrecondition(new SimplePrecondition(innerModelSink, new PredictionModelMetaData(PredictionModel.class, new ExampleSetMetaData())));				
		...
		getTransformer().addRule(new PassThroughRule(innerModelSink, modelOutput, true) {
			...
			public MetaData modifyMetaData(MetaData unmodifiedMetaData) {
				if (unmodifiedMetaData instanceof PredictionModelMetaData) {
					return modifyGeneratedModelMetaData((PredictionModelMetaData)unmodifiedMetaData);
				} else {
	...
	/**
	 * This method can be used by subclasses to additionally change the example set meta data delivered
	 * to the inner learner
	 */
