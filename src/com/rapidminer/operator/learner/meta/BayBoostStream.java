 ...
 * <li>base classifier weights are always adapted by a single row from first to
 * last</li>
...
public class BayBoostStream extends AbstractMetaLearner {
	...
	/**
	 * Class that filters an ExampleSet by the value of a special attribute. The
	 * constructor is provided with the attribute and the selected value.
	 * Neither this value nor the values of the attribute in the data are
	 * supposed to change. Please note, that the batch number is compared with
	 * greater or equal, which is comfortable for the case of merging a sequence
	 * of batches.
	 */
	...
	/**
	 * The probabilistic prediction of soft classifiers is restricted, similar
	 * to a confidence bound. If the lift is close to 0 it is replaced by the
	 * minimum lift below. Analogously a maximum lift value is defined by (1 /
	...
	public Model learn(ExampleSet exampleSet) throws OperatorException {
		...
		final Attribute streamControlAttribute;
		{
			...
			if ((attr = exampleSet.getAttributes().get(STREAM_CONTROL_ATTRIB_NAME)) == null)
				streamControlAttribute = com.rapidminer.example.Tools.createSpecialAttribute(exampleSet, STREAM_CONTROL_ATTRIB_NAME, Ontology.INTEGER);
			else {
	 ...
	 * shifts weights so that the priors are equal afterwards.
	 */
	...
	/**
	 * The preparation part collecting the examples of a batch, computing priors
	 * and resetting weights to 1.
	 * 
	 * @param currentBatchNum
	 *            the batch number to be assigned to the examples
	 * @param reader
	 *            the <code>Iterator<Example></code> with the cursor on the
	...
	/*
	 * Uses the training set (current batch) to update current model stored in
	 * modelInfo.
	 */
	...
	/**
	 * This helper method takes as input the traing set and the set of models
	 * trained so far. It re-estimates the model weights one by one, which means
	 * that it changes the contents of the modelInfo container. Works with crisp
	 * base classifiers, only!
	 * 
	 * @param exampleSet
	 *            the training set to be used to tune the weights
	 * @param modelInfo
	 *            the <code>Vector</code> of <code>Model</code>s, each with
	 *            its biasMatrix
	 * @return <code>true</code> iff the <code>ExampleSet</code> contains at
	...
	/**
	 * Helper method replacing
	 * <code>Model.createPredictedLabel(ExampleSet)</code> in order to lower
	 * memory consumption.
	 */
	...
	/**
	 * Adds the parameters &quot;rescale label priors&quot; and &quot;weighted
	 * batch size&quot;.
	 */
	@Override
	public List<ParameterType> getParameterTypes() {
		...
		types.add(new ParameterTypeDouble(PARAMETER_FRACTION_HOLD_OUT_SET, "Rel. size of hold out set for ensemble selection. Set to 0 to turn off.", 0, 1.0, 0));
