...
/**
 * This operator uses an inner learning scheme which is able to perform
 * predictions for binary or binominal classification problems and 
 * learns a set of these binary models in order to use this set for a
...
public class Binary2MultiClassModel extends PredictionModel implements MetaModel {
	...
	private void startNonECOCProcess (ExampleSet originalExampleSet, int classificationStrategy) throws OperatorException {
			...
			while (reader.hasNext()) {			
				...
				if (classificationStrategy == ONE_AGAINST_ONE) { 
					for (String className : exampleSet.getAttributes().getPredictedLabel().getMapping().getValues()) {
						confidenceMatrix[counter][getLabel().getMapping().mapString(className)] += example.getConfidence(className);
					}
				} else {
		...
		while (reader.hasNext()) {
			...
			for (int i = 0; i < numberOfClasses; i++) {
				example.setConfidence(getLabel().getMapping().mapIndex(classIndexMap.get(i)), confidenceMatrix[counter][i] / confidenceSum);					
			}
	...
	/**
	 * This method applies the models and evaluates the results of a multi class classification
	 * process according to a classification strategy that uses error-correcting output
	 * codes. (currently available: "exhaustive code", "random code")
	 * 
	 * @param originalExampleSet
	 * @param codeMatrix
	 * @throws OperatorException
	...
	private void startECOCProcess (ExampleSet originalExampleSet, String[][] codeMatrix) throws OperatorException {
		...
		// 2. Iterate over all models and all examples for every model to receive all confidence values.
		for (int k = 0; k < confidenceMatrix[0].length; k++) {
		...
		// 3. Iterate again over all examples to compute a prediction and a confidence distribution for 
