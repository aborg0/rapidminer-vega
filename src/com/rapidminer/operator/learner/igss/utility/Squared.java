...
public class Squared extends AbstractUtility {
	...
	public double conf (double totalWeight, double delta) {
		...
		return 3.0d/(2.0d*Math.sqrt(totalWeight))*inverseNormal +
			(totalWeight+Math.sqrt(totalWeight))/(4.0d*totalWeight*Math.sqrt(totalWeight))*Math.pow(inverseNormal,2.0d) + 
	...
	public double conf (double totalWeight, double totalPositiveWeight, Hypothesis hypo, double delta) {
		...
		return 2.0d*sg*inverseNormal+sg*sg*Math.pow(inverseNormal,2.0d) + sp*inverseNormal+
			2.0d*sg*sp*Math.pow(inverseNormal,2.0d) + sp*sg*sg*Math.pow(inverseNormal,3.0d);
	}
