...
public class IOConsumeOperator extends Operator {
	...
	private static final String[] DELETION_TYPES = new String[] { 
		"delete_one", "delete_all", "delete_all_but", "delete_all_but_number"
	};
	...
	@Override
	public void doWork() {
		getLogger().info("IOConsumer is deprecated and does nothing. It is only used while importing processes from earlier versions. After that, IOConsumers can be deleted.");
	}
			...
			switch (getParameterAsInt(PARAMETER_DELETION_TYPE)) {
				case DELETE_ONE:					
					...
					while (i.hasNext()) {
						...
						if (port.shouldAutoConnect() && (port.getMetaData() != null) && (clazz.isAssignableFrom(port.getMetaData().getObjectClass()))) {
							hits++;
							if (hits == number) {
								i.remove();
...
//					setInput(input.prepend(temp));
	...
	public List<ParameterType> getParameterTypes() {
		...
		types.add(type);
		type = new ParameterTypeCategory(PARAMETER_DELETION_TYPE, "Defines the type of deletion.", DELETION_TYPES, DELETE_ALL);
		type.setExpert(false);
		types.add(type);
