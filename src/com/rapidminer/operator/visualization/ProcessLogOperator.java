...
public class ProcessLogOperator extends Operator {
	...
	private void writeOnline(DataTableRow row) throws UserError {
		...
		try {
			// writing header if file does not exist or applyCount is 1 and file exists and has to be overwritten
			...
			if (dataTable.isNominal(sortingDimensionIndex)) {
				...
				for (int r = 0; r < dataTable.getNumberOfRows(); r++) {
					...
					if ((currentWorst == null) || 
							((sortingType == SORTING_TYPE_TOP_K ) && (currentNominalValue.compareTo(currentWorst) < 0)) ||
							((sortingType == SORTING_TYPE_BOTTOM_K) && (currentNominalValue.compareTo(currentWorst)) > 0)) {
				...
				if ((currentWorstIndex >= 0) && 
						((sortingType == SORTING_TYPE_TOP_K ) && (candidateNominalValue.compareTo(currentWorst) > 0)) ||
						((sortingType == SORTING_TYPE_BOTTOM_K) && (candidateNominalValue.compareTo(currentWorst)) < 0)) {
				...
				for (int r = 0; r < dataTable.getNumberOfRows(); r++) {
					double currentValue = dataTable.getRow(r).getValue(sortingDimensionIndex);	
					if ((Double.isNaN(currentWorst)) || 
							((sortingType == SORTING_TYPE_TOP_K ) && (currentValue < currentWorst)) ||
				...
				double candidateValue = dataRow.getValue(sortingDimensionIndex);	
				if ((currentWorstIndex >= 0) &&
						((sortingType == SORTING_TYPE_TOP_K ) && (candidateValue > currentWorst)) ||
	...
	public List<ParameterType> getParameterTypes() {
		...
		type = new ParameterTypeList(PARAMETER_LOG, "List of key value pairs where the key is the column name and the value specifies the process value to log.", 
