...
public class MultipleLabelGenerator extends AbstractExampleSource {
	...
	public MetaData getGeneratedMetaData() throws OperatorException {
		...
		if (getParameterAsBoolean(PARAMETER_REGRESSION)) {
			emd.addAttribute(new AttributeMetaData("label1", Attributes.LABEL_NAME + 1, Ontology.REAL, new Range(3 * lower, 3 * upper)));
			emd.addAttribute(new AttributeMetaData("label2", Attributes.LABEL_NAME + 2, Ontology.REAL, new Range(3 * lower, 3 * upper)));
			emd.addAttribute(new AttributeMetaData("label3", Attributes.LABEL_NAME + 3, Ontology.REAL, new Range(Math.max(lower, 0) * Math.max(lower, 0), upper * upper)));
		} else {
