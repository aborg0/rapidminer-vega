...
/**
 * This class can be used to use all pairs (entries) of a numerical matrix as data table. The data is directly
 * read from the numerical matrix instead of building a copy. If the matrix is symmetrical, only pairs of the lower left
 * triangle are returned.
 * Please note that the method for adding new rows is not supported by this type of data tables.
 * 
...
public class DataTablePairwiseMatrixExtractionAdapter extends AbstractDataTable {
    ...
    public DataTablePairwiseMatrixExtractionAdapter(NumericalMatrix matrix, String[] rowNames, String[] columnNames, String[] tableColumnNames, boolean showSymetrical) {
    ...
    public DataTableRow getRow(int rowIndex) {
        if (matrix.isSymmetrical() && showSymetrical) {
            ...
            return new PairwiseMatrix2DataTableRowWrapper(this.matrix, rowIndex / matrix.getNumberOfColumns(), rowIndex % matrix.getNumberOfColumns());
        }
    }
