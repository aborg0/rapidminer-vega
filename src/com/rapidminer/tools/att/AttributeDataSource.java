...
public class AttributeDataSource {
				...
				if (Ontology.ATTRIBUTE_VALUE_TYPE.isA(valueType, Ontology.NOMINAL)) {
					...
					if (classList.size() == 0) { // still empty class list? --> Warning
						if (type.equals(Attributes.ID_NAME)) {
							logging.logNote("The ID attribute '" + name + "' is defined with a nominal value type but the possible values are not defined! " +
							...
							logging.logError("The label attribute (class) '" + name + "' is defined with a nominal value type but the possible values are not defined! " +
	...
	public String toString() {
		return attribute.getName() + " (type: " + attributeType + ", value type: " + Ontology.VALUE_TYPE_NAMES[attribute.getValueType()] + ") from " + file.getName() + " (" + column + ")";
	}
}
