...
/**
 * A writer that will export the defined parameters that are preStart Parameters
 * to a config file in the user directory that is used by the exe start program
 * to set Environment Variables before actual JVM starts.
 * 
 * The format is line separated as follows:
 * - mode: Mode is either "append" or "overwrite". On Append the new value is appended ; separeted
...
public class WindowsExeParameterWriter implements ParameterWriter {
    ...
    public void writeParameters(Map<String, Parameter> parameters) {
        ...
        try {
            BufferedWriter writer = new BufferedWriter(new PrintWriter(configFile));
