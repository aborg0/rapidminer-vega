...
public abstract class ESOptimization implements Optimization {
	...
	public static final List<ParameterType> getParameterTypes(Operator parameterHandler) {
		...
		type = (new ParameterTypeInt(PARAMETER_GENERATIONS_WITHOUT_IMPROVAL, "Stop criterion: Stop after n generations without improval of the performance.", 1, Integer.MAX_VALUE, 2, false));
		...
		type = new ParameterTypeDouble(PARAMETER_TOURNAMENT_FRACTION, "The fraction of the population used for tournament selection.", 0.0d, Double.POSITIVE_INFINITY, 0.25d);
		type.registerDependencyCondition(new EqualTypeCondition(parameterHandler, PARAMETER_SELECTION_TYPE, ESOptimization.SELECTION_TYPES, true, ESOptimization.TOURNAMENT_SELECTION));
		types.add(type);
