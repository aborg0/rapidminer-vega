...
public class LiftDataGenerator {
	...
	public List<double[]> createLiftDataList(ExampleSet exampleSet) {
		...
		while (reader.hasNext()) {
			...
			if (weightAttr == null) {
				wcl = new WeightedConfidenceAndLabel((-1) * example.getConfidence(positiveClassName), example.getValue(labelAttr), example.getValue(predictedLabel));
			} else {
				wcl = new WeightedConfidenceAndLabel((-1) * example.getConfidence(positiveClassName), example.getValue(labelAttr), example.getValue(weightAttr), example.getValue(predictedLabel));
			}
			calArray[index++] = wcl;
		}
		...
		for (int i = 0; i < calArray.length; i++) {
			...
			double lift = 
				(confidenceMatrix[TP] * (confidenceMatrix[FP] + confidenceMatrix[TN])) /
