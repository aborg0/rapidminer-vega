...
/**
 * The RitterAdaptation provides an implementation of the AdaptationFunction interface for calculation the adaption of a
...
public class RitterAdaptation implements AdaptationFunction {
	...
	public double[] adapt(double[] stimulus, double[] nodeValue, double distanceFromImpact, int time, int maxtime) {
		...
		double distanceWeightCurrent = Math.exp((-distanceFromImpact * distanceFromImpact) / (2 * adaptationRadiusCurrent * adaptationRadiusCurrent));
	...
	private double getAdaptationRadius(int time, int maxtime) {
		return adaptationRadiusStart * Math.pow((adaptationRadiusEnd / adaptationRadiusStart), (((double) time) / ((double) maxtime)));
	}
