...
/**
 * Window functions apply a weight to each value of a value series depending on
 * the length of the series. Window Functions like Hanning windows are usually
 * applied before a Fourier transformation.
 * 
...
public class WindowFunction {
	...
	/**
	 * Returns the weighting factor for the current value n in a window of the
	 * given length. The calculation of this factor is done in dependance of the
	 * function type.
	 */
	public double getFactor(int n) {
		switch (type) {
			...
			case BLACKMAN_HARRIS:
				return 0.35875 - 0.48829 * Math.cos(2 * Math.PI * n / length) + 0.14128 * Math.cos(4 * Math.PI * n / length) - 0.01168 * Math.cos(6 * Math.PI * n / length);
			case BARTLETT:
