...
/**
 * This class is the model of plotter configuration. It holds the connection to the configuration data and all
 * listeners. It might be notified of changes of the parameters, updates the configuration data and informs all
 * listeners of the changed configuration.
 * 
...
public class PlotterConfigurationModel implements Cloneable {
	...
	public final static LinkedHashMap<String, Class<? extends Plotter>> COLOR_HISTOGRAM_PLOTTER_SELECTION = new LinkedHashMap<String, Class<? extends Plotter>>();
	...
	public static interface PlotterChangedListener {
		/**
		 * This has to return a complete list of the objects listening to the plotter settings, defined inside this
		 * class.
		 */
	...
	/**
	 * This is a convenience constructor building settings providing the complete plotter selection and creating the
	 * given plotter per default. This might be used, if exactly one plotter should be build.
	 */
	...
	public void setPlotter(String name) {
		if (settings.getPlotterName() == null || !settings.getPlotterName().equals(name) || isInitialPlotter) {
			...
			if (plotterClass != null && (isInitialPlotter || plotter == null || !(plotterClass.isAssignableFrom(plotter.getClass()) && plotter.getClass().isAssignableFrom(plotterClass)))) {
				isInitialPlotter = (plotter == null);
				Plotter plotter;
				try {
					plotter = settings.getAvailablePlotters().get(name).getConstructor(PlotterConfigurationModel.class).newInstance(this);
					setPlotter(plotter);
	...
	public void setPlotter(Plotter plotter) {
		...
		// this keeps old settings because they could be needed again and might be more useful than defaults
	...
	private void informAllPlotterListener(Plotter plotter) {
		List<PlotterChangedListener> copyPlotterListener = new ArrayList<PlotterChangedListener>(plotterListeners.size());
	...
	private void informAllSettingsListener() {
		...
		try {
			List<PlotterSettingsChangedListener> copySettingsChangedListener = new ArrayList<PlotterSettingsChangedListener>(settingsListeners.size());
	...
	public void setAxis(int axis, String column) {
		setParameterAsString(PlotterAdapter.PARAMETER_SUFFIX_AXIS + PlotterAdapter.transformParameterName(plotter.getAxisName(axis)), column);
	}
