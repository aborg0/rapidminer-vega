...
public class MultipleScatterPlotter extends LabelRotatingPlotterAdapter {
	...
	public MultipleScatterPlotter(final PlotterConfigurationModel settings) {
		...
		setBackground(Color.white);
     	...
     	pointsAndLinesOptions.setToolTipText("Opens a dialog where the draw type (points, lines, or points and lines) for each dimension can be specified.");
	...
	@Override
	public String getPlotName() { return "y-Axis"; }
	...
	public void updatePlotter() {
		...
		if (size <= 1) {
			...
			for (int i = 0; i < dataSet.getSeriesCount(); i++) {
				chart.getXYPlot().getRenderer().setSeriesStroke(i, new BasicStroke(1.5f, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND));
				chart.getXYPlot().getRenderer().setSeriesPaint(i, getColorProvider().getPointColor(i / (double)(dataSet.getSeriesCount() - 1)));
			}
		}
