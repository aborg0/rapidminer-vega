...
//							if (this.dataTable.getSelectionCount() == 0 || !dataTable.isDeselected(index)) {
	...
	protected void updatePlotter() {
		...
		if (histogramDataset.getSeriesCount() == 1) {
			...
			for (int i = 0; i < histogramDataset.getSeriesCount(); i++) {
				Color color = getColorProvider().getPointColor((double) i / (double) (histogramDataset.getSeriesCount() - 1));
...
//		renderer.setBarPainter(new StandardXYBarPainter());
	...
	public List<ParameterType> getAdditionalParameterKeys(InputPort inputPort) {
		...
		types.add(new ParameterTypeInt(PARAMETER_NUMBER_OF_BINS, "The number of bins for each histogram.", MIN_BIN_NUMBER, MAX_BIN_NUMBER, DEFAULT_BIN_NUMBER));
