...
public class BarChartPlotter extends LabelRotatingPlotterAdapter implements ChangeListener {
	...
	private static final String[] ORIENTATION_TYPES = new String[] {
		"vertical",
		"horizontal" 
	};
				...
				if (aggregation != null) {
					...
					while (c.hasNext()) {
						...
						if (groupByColumn < 0) {
							...
							//categoryDataSet.setValue(value, name, dataTable.getColumnName(valueColumn));
					...
					while (c.hasNext()) {
						...
						while (v.hasNext()) {
							...
							if (groupByColumn < 0) {
								...
								//categoryDataSet.setValue(value, name, dataTable.getColumnName(groupByColumn));
								categoryDataSet.setValue(value, dataTable.getColumnName(groupByColumn), name);
							}
						}
					}
	...
	public void updatePlotter() {	
		...
		if ((categoryCount > MAX_CATEGORY_VIEW_COUNT) && showScrollbar) {
			if (orientationIndex == ORIENTATION_TYPE_VERTICAL) {
				SwingUtilities.invokeLater(new Runnable() {
					public void run() {
						viewScrollBar.setOrientation(JScrollBar.HORIZONTAL);
				...
				SwingUtilities.invokeLater(new Runnable() {
					public void run() {
						viewScrollBar.setOrientation(JScrollBar.VERTICAL);
