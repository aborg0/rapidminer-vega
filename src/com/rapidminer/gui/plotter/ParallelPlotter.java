...
public class ParallelPlotter extends PlotterAdapter implements MouseListener {
	...
	public void paintParallelPlot(Graphics g) {
		...
		if ((colorColumn >= 0) && (colorColumn < min.length) && 
			...
			(dataTable.isNominal(colorColumn) || (min[colorColumn] != max[colorColumn])) && (dataTable.getNumberOfRows() > 0)) {
	...
	private void drawLines(Graphics g, int pixWidth, int pixHeight) {		
		...
		while (i.hasNext()) {
			...
			if (colorColumn != -1) {
				double colorValue = getColorProvider().getPointColorValue(this.dataTable, row, colorColumn, min[colorColumn], max[colorColumn]);
