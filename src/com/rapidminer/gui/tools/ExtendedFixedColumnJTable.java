...
/**
 * This table can be used to display a fixed column as the first column which will never be moved
 * or scrolled away. This can be nice if the first column contains names / ids for the rows which
...
public class ExtendedFixedColumnJTable extends JScrollPane {
	...
	private ExtendedFixedColumnJTable(ExtendedJTable mainDataTable, JViewport viewport, JTableHeader header) {
		// we have to manually attach the row headers, but after that, the scroll pane keeps them in sync
		super(mainDataTable);
