 ...
 * <p>
 * 
 * For performance reasons, the linked list of tokens is reused after each line is tokenized. Therefore, the return value of <code>markTokens</code> should only be used for immediate painting. Notably, it cannot be cached.
 * 
...
public abstract class TokenMarker {
	...
	/**
	 * An abstract method that splits a line up into tokens. It should parse the line, and call <code>addToken()</code> to add syntax tokens to the token list. Then, it should return the initial token type for the next line.
	 * <p>
	 * 
	...
	/**
	 * Returns if the token marker supports tokens that span multiple lines. If this is true, the object using this token marker is required to pass all lines in the document to the <code>markTokens()</code> method (in turn).
	 * <p>
	 * 
	...
	/**
	 * An array for storing information about lines. It is enlarged and shrunk automatically by the <code>insertLines()</code> and <code>deleteLines()</code> methods.
	 */
	...
	/**
	 * Creates a new <code>TokenMarker</code>. This DOES NOT create a lineInfo array; an initial call to <code>insertLines()</code> does that.
	 */
	...
	/**
	 * Ensures that the <code>lineInfo</code> array can contain the specified index. This enlarges it if necessary. No action is taken if the array is large enough already.
	 * <p>
	 * 
	 * It should be unnecessary to call this under normal circumstances; <code>insertLine()</code> should take care of enlarging the line info array automatically.
	 * 
	 * @param index
	 *            The array index
	 */
