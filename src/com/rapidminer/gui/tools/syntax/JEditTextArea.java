...
public class JEditTextArea extends JComponent implements MouseWheelListener {
	...
	public final transient Action COPY_ACTION = new ResourceActionTransmitter(true, "editor.copy", InputHandler.CLIP_COPY, this);
	
	public final transient Action CUT_ACTION = new ResourceActionTransmitter(true, "editor.cut", InputHandler.CLIP_CUT, this);
	
	public final transient Action PASTE_ACTION = new ResourceActionTransmitter(true, "editor.paste", InputHandler.CLIP_PASTE, this);
	...
	public final transient Action SELECT_ALL_ACTION = new ResourceActionTransmitter(true, "editor.select_all", InputHandler.SELECT_ALL, this);
	
	public final transient Action SEARCH_AND_REPLACE_ACTION = new ResourceActionTransmitter(true, "editor.search_replace", InputHandler.SEARCH_AND_REPLACE, this);
		...
		try {
			if (rectSelect) {
				...
				for (int i = selectionStartLine; i <= selectionEndLine; i++) {
					...
					currNewline = selectedText.indexOf(Tools.getLineSeparator(), lastNewline);
					if (currNewline == -1)
				...
				if (selectedText != null && currNewline != selectedText.length()) {
					...
					document.insertString(offset, Tools.getLineSeparator(), null);
	...
	public void paste() {
		if (editable) {
			...
			try {
				...
				// so do it here
				String selection = ((String) clipboard.getContents(this).getTransferData(DataFlavor.stringFlavor)).replace("\r", Tools.getLineSeparator());
