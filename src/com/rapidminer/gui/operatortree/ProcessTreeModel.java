 ...
 *  Rewrite of the original OperatorTreeModel.
 *  
 *  Nodes of this model can be {@link Operator}s and {@link ExecutionUnit}s. The latter
 *  are hidden from the model if an {@link OperatorChain} contains only one {@link ExecutionUnit}.
 * 
...
public class ProcessTreeModel implements TreeModel {
	...
	/** Returns either the list of enabled schildren of the given process
	 *  or the list of all children, depending on  {@link #showDisabled}. */
	...
	private TreeModelEvent makeChangeEvent(Operator operator) {
		...
		if (parent != null) {
			// NOTE: In the tree model, the parent may be an ExecutionUnit or an OperatorChain (if it has only one subprocess)
			// In both cases, the index of the operator is the same, so we don't have to treat these cases separately.
	...
	private void fireTreeNodesChanged(Operator operator) {		
		...
		if ((e.getChildIndices() != null) && (e.getChildIndices()[0] != -1)) { // otherwise the operator is in the state of being removed and has triggered an update while dying.
