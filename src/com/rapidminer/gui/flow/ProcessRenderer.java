...
public class ProcessRenderer extends JPanel {
	 ...
	 *  @param autoPositionIfMissing If true and no position has been defined yet, a position will be 
	 ...
	 *  this spacing is additional to the 3/2*PORT_SIZE raster used by
	 *  {@link #getPortLocation(Port)}. */
				...
				if (selectionRectangle != null) {
					if ((selectionRectangle.getWidth() > 3) && (selectionRectangle.getHeight() > 3)) {
						...
						if (offset != null) {
							...
							if ((!e.isShiftDown() && !e.isControlDown()) || 
									((selectedOperators.size() == 1) && (selectedOperators.get(0) == displayedChain))) { // if we have only selected the parent, we ignore SHIFT and CTRL
								selectedOperators.clear();
							}
...
//					mainFrame.addToUndoList();
		...
		public void mouseDragged(MouseEvent e) {
			...
			if ((draggedOperatorsOrigins != null) && !draggedOperatorsOrigins.isEmpty()) {
				...
				if (draggedOperatorsOrigins.size() == 1) {
					if (canBeInsertedIntoConnection(getHoveringOperator())) {
				...
				if (!draggedOperatorsOrigins.containsKey(getHoveringOperator())) {
					draggedOperatorsOrigins.put(getHoveringOperator(), getOperatorRect(getHoveringOperator(), false));
				}
				double targetX = draggedOperatorsOrigins.get(getHoveringOperator()).getX() + difX;
				double targetY = draggedOperatorsOrigins.get(getHoveringOperator()).getY() + difY;
				if (targetX < 0) {
					targetX = 0;
				}
				if (targetY < 0) {
				...
				difX = targetX - draggedOperatorsOrigins.get(getHoveringOperator()).getX();
				difY = targetY - draggedOperatorsOrigins.get(getHoveringOperator()).getY();
				...
				//scrollRectToVisible(new Rectangle(e.getX(), e.getY(), (int)opPosAtDragStart.getWidth(), (int)opPosAtDragStart.getHeight()));
	 ...
	 * @param clear If true, clear before adding
	 * @param range If true, select interval from last selected operator to op.
	 */
	...
	/**
	 * @param fill If true, use all free space in the parent viewport
	 */
