...
public class AssociationRulesGraphCreator extends GraphCreatorAdaptor implements AssociationRuleFilterListener {
	...
	private void addRuleNodes(boolean[] filter) {
		...
		for (int r = 0; r < rules.getNumberOfRules(); r++) {
			if (filter[r]) {
				...
				toolTipInfos.put(conjunctionNode, 
						"<html><b>Rule " + ruleIndex + "</b><br>" + SwingTools.addLinebreaks(rule.toPremiseString() + " --> " + rule.toConclusionString()) + 
						"<br><b>Support:</b> " + rule.getTotalSupport() + 
						"<br><b>Confidence:</b> " + rule.getConfidence() + 
						"<br><b>Lift:</b> " + rule.getLift() +
						"<br><b>Gain:</b> " + rule.getGain() +
						"<br><b>Conviction:</b> " + rule.getConviction() +
						"<br><b>Laplace:</b> " + rule.getLaplace() +
						"<br><b>Ps:</b> " + rule.getPs() +  
				"</html>");
	...
	public String getVertexToolTip(String id) {
    	...
    	if (toolTip != null) {
			...
			return
			"<html><b>Item:</b> " + id + 
			"<br><b>Premise in Rules:</b> " + (((asPremise.get(id) == null) || (asPremise.get(id).size() == 0)) ? "none" : asPremise.get(id)) + 
			"<br><b>Conclusion in Rules:</b> " + (((asConclusion.get(id) == null) || (asConclusion.get(id).size() == 0)) ? "none" : asConclusion.get(id)) + 
			"</html>";
    	}
    }
