...
public class TreeModelEdgeLabelRenderer<V, E> implements Renderer.EdgeLabel<V, E> {
	...
	public Component prepareRenderer(RenderContext<V, E> rc, EdgeLabelRenderer graphLabelRenderer, Object value, boolean isSelected, E edge) {
		return rc.getEdgeLabelRenderer().<E> getEdgeLabelRendererComponent(rc.getScreenDevice(), value, rc.getEdgeFontTransformer().transform(edge), isSelected, edge);
	}
	...
	public void labelEdge(RenderContext<V, E> rc, Layout<V, E> layout, E e, String label) {
		...
		if (!rc.getVertexIncludePredicate().evaluate(Context.<Graph<V, E>, V> getInstance(graph, v1)) || !rc.getVertexIncludePredicate().evaluate(Context.<Graph<V, E>, V> getInstance(graph, v2)))
			return;
		...
		/* BUG 1: change X and Y in distXXX
		xDisplacement = (int) (rc.getLabelOffset() * (distY / totalLength));
		yDisplacement = (int) (rc.getLabelOffset() * (-distX / totalLength));
		*/
