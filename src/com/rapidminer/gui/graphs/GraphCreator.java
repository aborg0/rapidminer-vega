...
/**
 * Creates a graph which can be displayed by the graph viewer. This class serves as a sort
...
public interface GraphCreator<V, E> {
    ...
    /** Returns the name that should be displayed in each vertex. May return null 
     *  (no name will be shown for this node). */
	public String getVertexName(V id);
	
    /** Returns the text that should be displayed as tool tip for each vertex. 
     *  May return null (no tool tip will be shown for this node). */
	public String getVertexToolTip(V id);

    /** Returns the text that should be displayed at the edge with the given id.  
     *  May return null (no text will be shown for this edge). */
	 ...
	 *  {@link #EDGE_SHAPE_QUAD_CURVE}, {@link #EDGE_SHAPE_CUBIC_CURVE}, {@link #EDGE_SHAPE_BENT_LINE},
	 *  or {@link #EDGE_SHAPE_WEDGE}. */
	public int getEdgeShape();
	
    /** Returns the minimal height for leafs. Might be important for graphs where the leafs
     *  should be rendered in some specialized way. May return -1 (no minimal height). */
    public int getMinLeafHeight();

    /** Returns the minimal width for leafs. Might be important for graphs where the leafs
     *  should be rendered in some specialized way. May return -1 (no minimal width). */
    ...
    /** Returns the viewer for objects. Will be notified after clicks on nodes. 
     *  May return null (no object viewer is used). */
    public GraphObjectViewer getObjectViewer();

    /** Returns the object for the given id, e.g. a cluster node. The result of this method
     *  will be given to the object viewer after a mouse click. */
    public Object getObject(V id);

    /** Returns the number of option componenents which will be added to the control panel of the {@link GraphViewer}. */
    public int getNumberOfOptionComponents();
    
    /** Returns the desired option componenents which will be added to the control panel of the {@link GraphViewer}. */
