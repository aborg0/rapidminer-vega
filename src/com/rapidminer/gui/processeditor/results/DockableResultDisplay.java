...
/** {@link ResultDisplay} that adds each result to an individual {@link Dockable}.
 *  In addition, it displays a result history overview.
 * 
...
public class DockableResultDisplay extends JPanel implements ResultDisplay {
		...
		desktop.addDockingActionListener(new DockingActionListener() {			
			...
			public boolean acceptDockingAction(DockingActionEvent e) {
				if ((e instanceof DockingActionCloseEvent) &&
					...
					return SwingTools.showConfirmDialog("result.really_close", ConfirmDialog.YES_NO_OPTION) == ConfirmDialog.YES_OPTION;
				} else {
					return true;
				}
			}
	...
	private void askForPerspectiveSwitch() {
		if (isAskingForPerspectiveSwitch || RapidMinerGUI.getMainFrame().getPerspectives().getCurrentPerspective().getName().equals("result")) {
			return;
		} else {
			try {
	...
	private void showResultNow(final ResultObject result, final String id) {
		ResultTab tab = (ResultTab)RapidMinerGUI.getMainFrame().getDockingDesktop().getContext().getDockableByKey(ResultTab.DOCKKEY_PREFIX+id);
		if (tab == null) {
	...
	private void updateDataTables() {
		final Collection<DataTable> copy = new LinkedList<DataTable>(dataTables.values());
		...
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				for (DataTableViewer viewer : viewers) {
					ProcessLogTab tab = (ProcessLogTab) RapidMinerGUI.getMainFrame().getDockingDesktop().getContext().getDockableByKey(ProcessLogTab.DOCKKEY_PREFIX + viewer.getDataTable().getName());
					if (tab == null) {
