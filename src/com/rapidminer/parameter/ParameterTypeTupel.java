 ...
 * {@link ParameterTypeList} or {@link ParameterTypeEnumeration}, these subtypes
 * are treated as one value.
 * In the gui these subtypes are shown beside each other.
 * 
...
public class ParameterTypeTupel extends CombinedParameterType {
    ...
    public ParameterTypeTupel(Element element) throws XMLException {
        ...
        for (Element entryElement: defaultEntryElements) {
            if (entryElement.hasAttribute(ATTRIBUTE_IS_NULL) && Boolean.valueOf(entryElement.getAttribute(ATTRIBUTE_IS_NULL)))
