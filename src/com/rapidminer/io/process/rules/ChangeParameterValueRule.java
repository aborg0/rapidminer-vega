	...
	protected String conditionedApply(Operator operator, String operatorTypeName, XMLImporter importer) {
		if (operator.getParameters().isSpecified(attributeName)) {
			...
			if (oldValue.equals(value)) {
				...
				return "The value of the parameter <code>" + attributeName + "</code> in <var>" + operator.getName() + "</var> (<code>" + operatorTypeName + "</code>) was changed from " + value + " to <code>" + newValue + "</code>.";
			}
		}
		return null;
	}
