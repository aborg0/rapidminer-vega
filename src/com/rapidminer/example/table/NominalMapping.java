 ...
 * and the internal double value representations which is used for nominal values
...
public interface NominalMapping extends Cloneable, Serializable {
	...
	/** Returns the index of a positive class (if available). Returns
	 *  -1 otherwise. */
	...
	/** Returns the index of a negative class (if available). Returns
	 *  -1 otherwise. */
	...
	/** Returns the internal double representation (actually an integer index) for the given
	 *  nominal value. This method creates a mapping if it did not exist before. */
	 ...
	 * contains {@link Example}s. All examples will be messed up otherwise!
	 */
