...
public class RepositoryActionConditionImplStandard implements RepositoryActionCondition {
	...
	/**
	 * Creates a new RepositoryActionCondition which can be used to check if the selected {@link Entry}s meet the given conditions.
	...
	public boolean evaluateCondition(List<Entry> entryList) {
		...
		for (Entry givenEntry : entryList) {
			...
			// make sure each entry's type is in the requiredTypes list, if not condition evaluates to false
			...
			// make sure each entry's repository type is in the requiredRepositoryTypes list, if not condition evaluates to false
